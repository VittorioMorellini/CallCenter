import typeToReducer from 'type-to-reducer';
import { Reducer } from 'redux';
import { useDispatch, useSelector } from 'react-redux';
import { EntityActions } from '../../framework/core/actions';
import { EntityLoaderInitialState, EntityReducer } from '../../framework/core/reducer';
import { EntityLoaderState } from '../../framework/core/types';
import { useLocalReducer } from '../../framework/hooks';
import { useLogger } from '../../framework/logger';
import { Logger } from '../../framework/logger/types';
import { RootState } from '../../app/reducers';
import { Category, CategorySearchModel } from '../../models/category';
import { CategoryService } from './service';

const key = 'CATEGORY';

const baseActions = (logger: Logger, dispatch: any, root: RootState) => {
    const base = EntityActions<Category, CategorySearchModel, RootState, CategoryService>({
        key,
        logger,
        ctor: Category,
        dispatch,
        service: root.service.category,
        getState: () => root
    });

    return base;
}

export const useCategoryActions = () => {

    const dispatch = useDispatch<any>();
    const logger = useLogger({ key });
    const state = useSelector((root: RootState) => root);
    
    return {
        actions: baseActions(logger, dispatch, state),
        logger
    };
}

export const useLocalCategoryActions = (initialState?: Partial<CategoryState>) => {

    const { state, dispatch } = useLocalReducer(categoryReducer, initialState);
    const logger = useLogger({ key });
    const rootState = useSelector((root: RootState) => root);

    return {
        actions: baseActions(logger, dispatch, rootState),
        logger,
        state
    };
}

export type CategoryState = EntityLoaderState<Category, CategorySearchModel>;

export const categoryReducer: Reducer<CategoryState, any> = typeToReducer({
    ...EntityReducer(key)
}, EntityLoaderInitialState({ searchModel: new CategorySearchModel() }));