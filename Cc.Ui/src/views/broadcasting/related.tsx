...
function RelatedView ({ idMaster, items }: Props) {
    ...
    const handler = {
        itemClick: (item: Broadcasting, index: number) => {
            actions.itemSelected(item, index);
            navigate('/broadcasting/' + item.id);
        },
        add: () => {
            let x = Broadcasting.newItem();
            // x.masterId = idMaster;
            handler.itemClick(x, 0);
        },
        save: (item: Broadcasting) => {
    ...
    return (
        <Panel 
            title={t('views:broadcasting.index.title')}
